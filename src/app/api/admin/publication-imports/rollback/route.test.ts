...
describe("POST /api/admin/publication-imports/rollback", () => {
  it("requires endpoint configuration", async () => {
    ...
    try {
      const response = await POST(
        new Request("https://pnpu.mes.gob.cu/api/admin/publication-imports/rollback", {
          method: "POST",
  ...
  it("validates auditId before configuration-dependent work", async () => {
    ...
    try {
      const response = await POST(
        new Request("https://pnpu.mes.gob.cu/api/admin/publication-imports/rollback", {
          ...
          headers: {
            ...
            "X-Correlation-Id": "rollback-request-1",
          },
      ...
      await expect(response.json()).resolves.toEqual({
        ...
        correlationId: "rollback-request-1",
      });
