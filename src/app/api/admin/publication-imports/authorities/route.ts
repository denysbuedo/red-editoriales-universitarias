...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    return authoritiesErrorResponse(request, error);
  }
}
...
function authorizeRequest(request: Request): NextResponse | null {
  ...
  if (request.headers.get("X-PNPU-Admin-Token") !== configuredToken) {
    return NextResponse.json(
      {
        code: "PNPU-403",
        message: "Publication import authorities token is invalid.",
      },
      { status: 403 },
    );
  }
...
function authoritiesErrorResponse(request: Request, error: unknown): NextResponse {
  ...
  const response =
    error instanceof ApplicationError
      ...
      : NextResponse.json(
          {
            code: "PNPU-503",
            message: "Publication import authorities lookup failed.",
            correlationId,
          },
          { status: 503 },
        );
