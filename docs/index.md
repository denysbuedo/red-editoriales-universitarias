# Plataforma Nacional de Publicaciones Universitarias

Este sitio reúne la documentación técnica operativa de la PNPU.

## Documentos

- [Fundación técnica](release-0-foundation.md).
- [Readiness operativo v0.1](release-0.1-readiness.md).
- [Linea base de produccion v0.1](production-baseline-v0.1.md).
- [Reporte de aceptacion v0.1](release-0.1-acceptance-report.md).
- [Despliegue Ubuntu](deploy_ubuntu.md).
- [Backup operativo minimo](backup-operational-runbook.md).
- [Hardening inicial v0.1](hardening-v0.1.md).
- [Runbook de identidad administrativa](identity-admin-runbook.md).
- [Theme Reduniv para Keycloak](keycloak-reduniv-theme.md).
- [Documento de diseño de producto](PDD_Plataforma_Nacional_Publicaciones_Universitarias_v0.1.md).
- [Especificación técnica del portal](Especificacion_Tecnica_Portal_RNEU.md).
- [Manual del administrador PNPU](manual-admin-pnpu.md).
- [Manual para responsables de editoriales](manual-responsables-editoriales.md).
- [Propuesta de registro editorial e importacion](editorial-registry-and-import-proposal.md).
- [Propuesta de importacion controlada hacia Omeka S](omeka-controlled-import-proposal.md).

## Versiones HTML para operación

- [Manual del administrador PNPU](manuales-html/manual-admin-pnpu.html).
- [Manual para responsables de editoriales](manuales-html/manual-responsables-editoriales.html).

La arquitectura empresarial aprobada se mantiene en las carpetas numeradas del repositorio y no se reorganiza desde este sitio.
